...
{
	if(array_key_exists('1001',$result_array)){$mrd=$result_array['1001'];}else{$mrd='';}
	if(array_key_exists('1002',$result_array)){$name=$result_array['1002'];}else{$name='';}
	if(array_key_exists('1003',$result_array)){$request_id=$result_array['1003'];}else{$request_id='';}
	...
	echo '<div class="basic_form">';
		...
		echo '	<label  class="my_label text-danger" for="name">Name</label>
				<input class="form-control text-danger" type=text required="required" pattern="[a-zA-Z\s]{2,}" 
				id=name name=name placeholder=name value=\''.$name.'\'>
		...
		echo '	<label  class="my_label" for="group_id">Request ID</label>
				...
				id=request_id name=request_id placeholder=request_id value=\''.$request_id.'\'>
